...
int countCoincidentSubscriptions(PGconn *conn, int theSubscriberPhone)
{
    ...
    // This solution uses the views that are in createNewspaperViews.sql, and is much easier to write and read
    sprintf(coincident_subscription_count_query, \
            "SELECT COUNT(*) \
             FROM DifferentCoincidentSubscriptionDates \
             WHERE CommonSubscriberPhone = %d \
            ", theSubscriberPhone);
...
int changeAddresses(PGconn *conn, char *oldAddress, char *newAddress)
{
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK){
        fprintf(stderr, "COMMIT TRANSACTION failed for changeAddresses\n");
        PQclear(res);
        bad_exit(conn);
    }
    PQclear(res);
